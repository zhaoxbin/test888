...
void RW_NDEF_T4T_Reset(void)
{
    ...
    RW_NDEF_T4T_Ndef.pMessage = NdefBuffer_IMMO;
}

/*
 * 函数名称：RW_NDEF_T4T_Get_KeyLen
 * 函数功能：获取的汽车钥匙数据长度
 * 形式参数：无
 * 返回参数：数据长度
 */
...
{
	if(Reading_NDEF_FINISH == eRW_NDEF_T4T_State)
	{
		return RW_NDEF_T4T_Ndef.MessageSize;
	}
	else
	{
		return 0;
...
void RW_NDEF_T4T_Read_Next(unsigned char *pRsp, unsigned short Rsp_size, unsigned char *pCmd, unsigned short *pCmd_size)
{
    ...
    *pCmd_size = RW_NDEF_T4T_OPERATION_NONE;
    ...
    case Selecting_NDEF_Application10:
		...
		{
			eRW_NDEF_T4T_State = Selecting_CC_Fail;
		}
        break;
    ...
    case Reading_CC:
		...
		{
			eRW_NDEF_T4T_State = Reading_CC_Fail;
		}
        break;
    ...
    case Selecting_NDEF:
		...
		{
			eRW_NDEF_T4T_State = Selecting_NDEF_Fail;
		}
        break;
    ...
    case Reading_NDEF_Size:
        ...
        if (!memcmp(&pRsp[Rsp_size - 2], RW_NDEF_T4T_OK, sizeof(RW_NDEF_T4T_OK)))
        {
            ...
            {
            	*pCmd_size = RW_NDEF_T4T_FAULT_OVERSIZE;
		...
		{
			eRW_NDEF_T4T_State = Reading_NDEF_Size_Fail;
		}
    ...
    case Reading_NDEF:
        ...
        if (!memcmp(&pRsp[Rsp_size - 2], RW_NDEF_T4T_OK, sizeof(RW_NDEF_T4T_OK)))
        {
            ...
            {
            	eRW_NDEF_T4T_State = Reading_NDEF_FINISH;
            	*pCmd_size = RW_NDEF_T4T_READ_FINSH;
		...
		{
			eRW_NDEF_T4T_State = Reading_NDEF_Fail;
		}
        break;
